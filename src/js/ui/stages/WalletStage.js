...
export default class WalletPage extends UIStage {
    ...
    async renderAssets(parentEl, lq, filter, ui) {
        ...
        let i=0;
        await Promise.all(balance.map(balance => {
            
            const id = "asset" + balance.asset.substr(0, 8) + balance.asset.substr(-8);

            const assetEl = Html.$list(assetsEl, "#" + id, ["asset", "l$h", "p$v", "p$center","l$right"]);
            ...
            lq.v(balance.value,balance.asset).human().then((value) => {
                balanceEl.setValue(value);
            });

            lq.v(balance.value,balance.asset).human(lq.getBaseAsset()).then((price) => {
                if (price) {
            ...
            lq.v(balance.value, balance.asset).human("USD").then((price) => {
                if (price) {
            ...
            const loadInfoPromise = lq.assets().getAssetInfo(balance.asset).then((info) => {
                try {
                    if (filter) {
                        if (!filter(balance.hash, true) && !filter(info.ticker) && !filter(info.name)) {
                            assetEl.remove();
                            return;
                        }
                    }
            ...
            lq.getTransaction(tx.tx_hash).then((txData) => {
                ...
                if (!txData.info.valid) {
                    ...
                    if (txData.info.isIncoming) {
                        ...
                        lq.assets().getAssetInfo(txData.info.outAsset).then(info => {
                            if (filter) {
                                if (!filter(info.hash, true) && !filter(info.ticker) && !filter(info.name) && !filter(tx.tx_hash, true)) {
                                    txElCnt.remove();
                                    return;
    ...
    async renderSendReceive(parentEl, lq, filter, ui) {
        ...
        Html.$button(cntEl, ".receive", []).setValue("Receive").setIconValue("arrow_downward").setAction(()=>{
            ui.setStage("receive");
        });
        Html.$button(cntEl, ".send", []).setValue("Send").setIconValue("arrow_upward").setAction(() => {
            ui.setStage("send");
        });
    }
